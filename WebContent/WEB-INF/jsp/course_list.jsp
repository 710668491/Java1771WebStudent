...
<html>
	...
	function delcourse(id) {
		...
		if(isDel) {
			location.href = "${ctx}/course?method=delete&id=" + id;  
		}
	...
	<body>
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-2">
					<div class="list-group">
					  <a href="${ctx}/course?method=findcourse" class="list-group-item active ">课程列表</a>
					  <a href="${ctx}/course?method=getcourseAdd" class="list-group-item ">课程添加</a>
					</div>
				</div>
				<!--左边部分 end-->
				<!--右边部分 Benin-->
			<div class="col-md-10">
			<!-- 搜索表单begin -->
					<form action="${ctx}/course?method=searchByCondition" method="post" id="searchForm">
					<input type="hidden" name="pageNo" id="pageNo">
						
						科目：<input type="text" name="name" value="${searchCondition.name}"/>
				       ...
				       <tbody>
				      	<c:forEach items="${pageBean.list}" var="course">
					        <tr>
					          <td>${course.id}</td>
					          <td>${course.name}</td>
					          <td><a href="javascript:delcourse(${course.id})" class="btn btn-warning" >删除</a></td>
			                  <td><a href="${ctx}/course?method=toUpdate&id=${course.id}" class="btn btn-info">修改</td>
					        </tr>
				      	</c:forEach>
				      </tbody>
				      </table>
